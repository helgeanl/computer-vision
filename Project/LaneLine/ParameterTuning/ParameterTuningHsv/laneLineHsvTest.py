...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    try:
        if not lines:
            ...
            HOUGH_IMG = line_img
            return line_img
    except: #Truth value of array is ambigous and an exception is raised
...
def printAll():
    ...
    mplt.show()
    mplt.imshow(HSV_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(COLORFILTERED_IMG, cmap='gray')
    mplt.show()
...
def saveAll():
	...
	mimg.imsave('frame'+str(frameCount)+'_1Hsv.jpg',HSV_IMG)
	mimg.imsave('frame'+str(frameCount)+'_2Clrfltrd.jpg',COLORFILTERED_IMG)
...
def processImage(base_img):
    global BASE_IMG, HSV_IMG, COLORFILTERED_IMG, CANNY_IMG, REGION_IMG, HOUGH_IMG, WEIGHT_IMG, frameCount
    BASE_IMG = base_img
