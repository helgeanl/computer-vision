...
def printAll():
    mplt.imshow(BASE_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(GRAY_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(CANNY_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(REGION_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(HOUGH_IMG, cmap='gray')
    mplt.show()
    mplt.imshow(WEIGHT_IMG, cmap='gray')
    mplt.show()

def saveAll():
    mimg.imsave('frame'+str(frameCount)+'_0Base.jpg',BASE_IMG)
    mimg.imsave('frame'+str(frameCount)+'_1Gray.jpg',GRAY_IMG)
    mimg.imsave('frame'+str(frameCount)+'_3Canny.jpg',CANNY_IMG)
    mimg.imsave('frame'+str(frameCount)+'_4RegionOfI.jpg',REGION_IMG)
    mimg.imsave('frame'+str(frameCount)+'_5Hough.jpg',HOUGH_IMG)
    mimg.imsave('frame'+str(frameCount)+'_6Finished.jpg',WEIGHT_IMG)
...
def weightImage(img, initial_img, α=1., β=1., λ=0.):
    ...
    # mplt.imshow(initial_img, cmap='gray')
    # mplt.show()
    added_img = cv2.addWeighted(initial_img, α, img, β, λ)
    # print('Displaying the images added together')
    # mplt.imshow(added_img, cmap='gray')
    # mplt.show()    
    return added_img


def processImage(base_img):
    global BASE_IMG, GRAY_IMG, CANNY_IMG, REGION_IMG, HOUGH_IMG, WEIGHT_IMG, frameCount
    ...
    GRAY_IMG = image
    image = cannyEdgeDetection(image, 10, 50)
    CANNY_IMG = image
    
    image = region_of_interest(
        image,
        np.array(
            [[(40, ysize), (xsize / 2 - xsize/10, ysize / 2 + 40), (xsize / 2 + xsize/10, ysize / 2 + 40), (xsize - 40, ysize)]],
            dtype=np.int32
        )
    )
    REGION_IMG = image
    ...
    WEIGHT_IMG = weightImage(image, base_img, β=250.)

    if frameCount % 30 == 0:
        pass
        #printAll()
        saveAll()
    if frameCount == 999999999999:
        print('This is a very long video, resetting frameCount to 0\n')
        frameCount = 0
    else:
        frameCount += 1
